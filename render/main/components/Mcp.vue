...
async function copyServerJson(server) {
    try {
        ...
        const copyResult = await window.api.copyText(jsonString);

        if (copyResult?.success === false) {
            throw new Error(copyResult?.message || copyResult?.error || 'copy_failed');
        }

        ElMessage.success(t('mcp.alerts.configCopied'));
    } catch (error) {
        console.error('[MCP] copyServerJson failed:', error);
        ElMessage.error(t('mcp.alerts.saveFailed'));
    }
}
